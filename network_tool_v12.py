...
class NetworkScanner:
    ...
    def _create_ui(self):
        ...
        tk.Label(header, text="🌊 Network Scanner Pro", bg=COLORS['accent'], 
                fg='white', font=('Segoe UI', 14, 'bold')).pack(side=tk.LEFT, padx=15)
        
        tk.Label(header, text="v3.0", bg=COLORS['accent'], 
                fg='#A0D0FF', font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=(0, 15))
        
        tk.Label(header, text="📧 mrbuivanvn@gmail.com", bg=COLORS['accent'], 
                fg='white', font=('Segoe UI', 9)).pack(side=tk.RIGHT, padx=15)
        
        self.status = tk.Label(header, text="✓ Ready", bg=COLORS['accent'], 
        ...
        # Sidebar
        sidebar = tk.Frame(main, bg=COLORS['sidebar'], width=160)
        sidebar.pack(side=tk.LEFT, fill=tk.Y)
        sidebar.pack_propagate(False)
        
        tk.Label(sidebar, text="🌊\nNETWORK\nSCANNER", bg=COLORS['sidebar'], 
                fg=COLORS['accent'], font=('Segoe UI', 11, 'bold'), justify=tk.CENTER).pack(pady=(15, 15))
        ...
        for text, cmd in buttons:
            btn = tk.Button(sidebar, text=text, bg=COLORS['sidebar'], fg=COLORS['text'],
                          ...
                          anchor=tk.W, padx=15, pady=12, bd=0,
    ...
    def _show_scan(self):
        ...
        tk.Label(range_frame, text="IP Range:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        self.scan_range = tk.Entry(range_frame, bg=COLORS['card'], fg=COLORS['text'], 
                                   width=22, font=('Consolas', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        self.scan_range.pack(side=tk.LEFT, padx=5)
        self.scan_range.insert(0, "192.168.1.1-192.168.1.254")
        
        tk.Label(range_frame, text="CIDR:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        self.scan_cidr = tk.Entry(range_frame, bg=COLORS['card'], fg=COLORS['text'], 
                                  width=18, font=('Consolas', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        ...
        style.configure('Treeview', background=COLORS['card'], foreground=COLORS['text'], 
                       fieldbackground=COLORS['card'], rowheight=24, borderwidth=0)
        style.configure('Treeview.Heading', background=COLORS['sidebar'], 
                       foreground=COLORS['text'], font=('Segoe UI', 9, 'bold'), borderwidth=1)
        ...
        self.scan_info = tk.Label(self.content, text="", bg=COLORS['bg'], fg=COLORS['text_sec'], font=('Segoe UI', 9))
        self.scan_info.pack(fill=tk.X, pady=(5, 0))
    ...
    def _do_scan(self):
        ...
        def scan_ip(ip):
            try:
                ...
                timeout_val = '-w 500' if IS_WINDOWS else '-W 500'
                
                start = time.time()
                result = subprocess.run(['ping', param, '1', timeout_val, ip], 
        ...
        def do():
            ...
            try:
                ...
                def update():
                    ...
                    self.scan_info.config(text=f"✓ Scan completed - Found {len(results)} devices")
                    self.status.config(text=f"✓ Ready")
                    self.scanning = False
    ...
    def _get_mac(self, ip):
        try:
            ...
            out = subprocess.check_output(cmd, universal_newlines=True, timeout=2,
                                        stderr=subprocess.DEVNULL)
    ...
    def _scan_ping_selected(self):
        ...
        if sel:
            ...
            self._open_cmd(f'ping {ip} -t' if IS_WINDOWS else f'ping {ip}')
    ...
    def _show_port(self):
        ...
        tk.Label(toolbar, text="Target:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        ...
        tk.Label(toolbar, text="Ports:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        self.port_list = tk.Entry(toolbar, bg=COLORS['card'], fg=COLORS['text'], 
                                  width=35, font=('Consolas', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
    ...
    def _show_ping(self):
        ...
        tk.Label(add_frame, text="IP:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        self.ping_ip = tk.Entry(add_frame, bg=COLORS['card'], fg=COLORS['text'], 
                                width=18, font=('Consolas', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        self.ping_ip.pack(side=tk.LEFT, padx=5)
        
        tk.Label(add_frame, text="Name:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 9)).pack(side=tk.LEFT, padx=5)
        self.ping_name = tk.Entry(add_frame, bg=COLORS['card'], fg=COLORS['text'], 
                                  width=15, font=('Segoe UI', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        self.ping_name.pack(side=tk.LEFT, padx=5)
        
        tk.Button(add_frame, text="Add", bg=COLORS['accent'], fg='white',
                 padx=15, pady=6, command=self._add_ping, relief=tk.FLAT, bd=0,
                 activebackground=COLORS['accent_hover']).pack(side=tk.LEFT, padx=10)
        tk.Button(add_frame, text="Remove All", bg=COLORS['error'], fg='white',
                 padx=15, pady=6, command=self._remove_all_ping, relief=tk.FLAT, bd=0,
                 activebackground='#DA3633').pack(side=tk.LEFT)
        
        list_frame = tk.LabelFrame(self.content, text="Saved Hosts", bg=COLORS['card'], 
                                   fg=COLORS['text'], font=('Segoe UI', 9, 'bold'), borderwidth=1)
        ...
        tk.Button(btn_frame, text="▶ Ping Selected", bg=COLORS['success'], fg='white',
                 ...
                 activebackground='#2EA043').pack(side=tk.LEFT, padx=5)
        tk.Button(btn_frame, text="❌ Remove Selected", bg=COLORS['error'], fg='white',
                 ...
                 activebackground='#DA3633').pack(side=tk.LEFT, padx=5)
    ...
    def _ping_selected(self):
        ...
        if sel:
            ...
            self._open_cmd(f'ping {host["ip"]} -t' if IS_WINDOWS else f'ping {host["ip"]}')
    ...
    def _show_trace(self):
        ...
        self.trace_ip = tk.Entry(frame, bg=COLORS['card'], fg=COLORS['text'], 
                                 width=30, font=('Consolas', 11), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        ...
        tk.Button(frame, text="Trace Route", bg=COLORS['accent'], fg='white',
                 font=('Segoe UI', 10, 'bold'), padx=25, pady=8,
    ...
    def _show_vnc(self):
        ...
        self.vnc_ip = tk.Entry(frame, bg=COLORS['card'], fg=COLORS['text'], 
                               width=20, font=('Consolas', 11), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        self.vnc_ip.pack(side=tk.LEFT, padx=5)
        self.vnc_ip.insert(0, "192.168.1.10")
        
        tk.Label(frame, text="Port:", bg=COLORS['bg'], fg=COLORS['text'], font=('Segoe UI', 11)).pack(side=tk.LEFT, padx=5)
        self.vnc_port = tk.Entry(frame, bg=COLORS['card'], fg=COLORS['text'], 
                                 width=8, font=('Consolas', 11), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
    ...
    def _show_groups(self):
        ...
        self.new_group_name = tk.Entry(top, bg=COLORS['card'], fg=COLORS['text'], 
                                       width=20, font=('Segoe UI', 10), relief=tk.FLAT, bd=1, insertbackground=COLORS['accent'])
        self.new_group_name.pack(side=tk.LEFT, padx=5)
        tk.Button(top, text="Create Group", bg=COLORS['accent'], fg='white',
                 padx=15, pady=6, command=self._create_group, relief=tk.FLAT, bd=0,
                 activebackground=COLORS['accent_hover']).pack(side=tk.LEFT, padx=10)
        ...
        self.groups_list = tk.Listbox(left, bg=COLORS['bg'], fg=COLORS['text'], 
                                      selectbackground=COLORS['accent'], font=('Segoe UI', 10), borderwidth=0)
        ...
        tk.Button(btn_frame, text="Add IP", bg=COLORS['accent'], fg='white',
                 ...
                 activebackground=COLORS['accent_hover']).pack(side=tk.LEFT, padx=5)
    ...
    def _show_backup(self):
        ...
        tk.Button(frame, text="📤 Export Configuration", bg=COLORS['accent'], fg='white',
                 font=('Segoe UI', 11, 'bold'), padx=40, pady=12, command=self._export,
                 relief=tk.FLAT, bd=0, activebackground=COLORS['accent_hover']).pack(pady=10)
        tk.Button(frame, text="📥 Import Configuration", bg=COLORS['accent'], fg='white',
                 font=('Segoe UI', 11, 'bold'), padx=40, pady=12, command=self._import,
                 relief=tk.FLAT, bd=0, activebackground=COLORS['accent_hover']).pack(pady=10)
        tk.Button(frame, text="🔄 Reset All Settings", bg=COLORS['error'], fg='white',
                 font=('Segoe UI', 11, 'bold'), padx=40, pady=12, command=self._reset,
                 relief=tk.FLAT, bd=0, activebackground='#DA3633').pack(pady=10)
    ...
    def _export(self):
        ...
        if f:
            ...
            self.status.config(text="Configuration exported")
    ...
    def _import(self):
        ...
        if f:
            try:
                ...
                self.status.config(text="Configuration imported")
            except Exception as e:
    ...
    def _reset(self):
        if messagebox.askyesno("Confirm", "Reset all settings?"):
            ...
            self.status.config(text="All settings reset")
    ...
    def _open_cmd(self, command):
        """Open command in terminal/cmd"""
        if IS_WINDOWS:
